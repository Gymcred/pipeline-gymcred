...
def n(v):
    try:
        f = float(v)
        return 0 if pd.isna(f) else round(f, 4)
    except:
        return 0
...
for _, r in df_p.iterrows():
    dd = None
    if pd.notna(r.get('data_desembolso', None)):
        try: dd = pd.to_datetime(r['data_desembolso']).strftime('%Y-%m-%d')
        except: dd = None
    pipeline.append({
        ...
        'financiado': n(r['financiado']), 'dataDesembolso': dd,
...
for _, r in df_c.iterrows():
    ...
    carteira.append({
        ...
        'valor': valor, 'vp': vp,
        'vpVsValor': round((vp/valor - 1)*100, 1) if valor > 0 else 0,
...
print(f"✅ index.html gerado com {len(pipeline)} ops pipeline, {len(carteira)} ops carteira")
